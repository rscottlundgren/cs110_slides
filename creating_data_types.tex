...
\begin{frame}[fragile]
\begin{framed}
\tiny \lstinline{koch.py}: Accepts integer \carg{n} as a command-line argument and plots a Koch curve of order \carg{n}.
\end{framed}
...
\begin{frame}[fragile]
\begin{framed}
\tiny \lstinline{spiral.py}: Accepts command-line arguments \carg{n} (an integer specifying a number  of sides), \carg{wraps} (an integer specifying a wrap count), and \carg{decay} (a float specifying a decay factor), and draws a spiral with those characteristics.
\end{framed}
...
\begin{frame}[fragile]
\begin{framed}
\tiny \lstinline{drunk.py}: Accepts as command-line arguments an integer \carg{stepCount} specifying the number of iterations, and a float \carg{stepSize} specifying a step size, creates a \lstinline{Turtle} object, and has it make \carg{stepCount} random steps of size \carg{stepSize}.
\end{framed}
...
\begin{frame}[fragile]
\begin{framed}
\tiny \lstinline{drunks.py}: Accepts as command-line arguments an integer \carg{turtleCount}, an integer \carg{stepCount}, and a float \carg{stepSize}, creates \carg{turtleCount} \lstinline{Turtle} objects, and has them make \carg{stepCount} random steps of size \carg{stepSize}.
\end{framed}
...
\item Addition: $(x+yi)+(v+wi) = (x+y) + (y+w)i$.
...
\begin{frame}[fragile]
\begin{framed}
\tiny \lstinline{mandelbrot.py}: Accepts float command-line arguments \carg{xc}, \carg{yc}, and \carg{size} that specify the center and size of a square region of interest, and makes a digital image showing the result of sampling the Mandelbrot set in that region. The algorithm is as follows:
\begin{itemize}
