...
\begin{frame}[fragile]
\begin{framed}
\tiny \lstinline{chargeclient.py}: Accepts floats \carg{x} and \carg{y} as command-line arguments, creates two \lstinline{Charge} objects with fixed position and electrical charge, and writes to standard output the potential at $(\carg{x}, \carg{y})$ due to the two charges.
\end{framed}
...
\begin{frame}[fragile]
\begin{framed}
\tiny \lstinline{alberssquares.py}: Accepts integers \carg{r1}, \carg{g1}, \carg{b1}, \carg{r2}, \carg{g2}, and \carg{b2} as command-line arguments, and draws to standard draw Albers squares using colors $(\carg{r1}, \carg{g1}, \carg{b1})$ and $(\carg{r2}, \carg{g2}, \carg{b2})$.
\end{framed}
...
\begin{frame}[fragile]
\begin{framed}
\tiny \lstinline{scale.py}: Accepts the name \carg{fileName} of a JPG or PNG image file, an integer \carg{w}, and an integer \carg{h} as command line arguments, reads an image from the file, and draws to standard draw the image scaled to width \carg{w} and height \carg{h}.
\end{framed}
...
\begin{frame}[fragile]
\begin{framed}
\tiny \lstinline{fade.py}: Accepts strings \carg{sourceFile} and \carg{targetFile} and integer \carg{n} as command-line arguments, and then, over the course of \carg{n} frames, gradually replaces the image from \carg{sourceFile} with the image from \carg{targetFile}, and displays to standard draw each intermediate image. 
\end{framed}
...
\begin{frame}[fragile]
\begin{framed}
\tiny \lstinline{stockquote.py}: Accepts string \carg{stockSymbol} as a command-line argument, and writes to standard output the current stock price for \carg{stockSymbol}, as reported by the website \href{http://finance.yahoo.com/}{http://finance.yahoo.com/}.
\end{framed}
...
\begin{frame}[fragile]
\begin{framed}
\tiny \lstinline{split.py}: Accepts string \carg{fileName} and integer \carg{fieldCount} as command-line arguments, splits the file whose name is \carg{fileName}.csv, by field, into \carg{fieldCount} files named \carg{fileName}0.txt, \carg{fileName}1.txt, etc.
\end{framed}
