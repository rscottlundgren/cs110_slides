...
\begin{center}
\begin{tabular}{c|c}
values & sequences of characters \\ \hline
...
The two \lstinline{bool} literals can be represented as \lstinline{True} and \lstinline{False}.
...
You can call a function in your code by typing its name followed by \emph{arguments}, enclosed in parentheses and separated by commas. For example, \lstinline{math.sqrt(2.0)}.

\bigskip

When Python executes your program, we say that it \emph{calls} (or \emph{evaluates}) the function with the given arguments and that the function \emph{returns} a value.

\bigskip

A function call is an expression, so you can use a function call in the same way that you use variables and literals to build up more complicated expressions. For example, \lstinline{math.sin(x) * math.cos(y)}. 

\bigskip 

You can also represent function arguments as expressions. For example, \lstinline{math.sqrt(b * b - 4.0 * a * c)}.
\end{frame}
...
\lstinline$min(a, b)$ & minimum value of $a$ and $b$ \\
$\dots$ & $\dots$
\end{tabular} 
\end{center}
...
Standard functions from Python's \lstinline{math} module:
...
\lstinline$math.tan(x)$ & tangent of $x$ (expressed in radians) \\
$\dots$ & $\dots$
\end{tabular} 
\end{center}
