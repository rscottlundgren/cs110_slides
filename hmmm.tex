...
Instead of programming in binary (0's and 1's), we'll use \emph{assembly language}, a programming language where each instruction has a symbolic representation; For example, to compute \lstinline{r3 = r1 + r2}, we'll write \lstinline{add r3 r1 r2}
...
\begin{framed}
\tiny combinations.hmmm: Calculate $C(N, K)$ (aka $N$ choose $K$) defined as $C(N, K)=N!/(K!(N-K)!)$.
\end{framed}
